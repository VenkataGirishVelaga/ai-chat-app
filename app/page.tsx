  ...
  const handleSend = async () => {
    ...
    setChats((prev) =>
      prev.map((chat) => {
        ...
        userMsgIndex = chat.messages.length; // index it will land at
        return { ...chat, messages: [...chat.messages, userMessage] };
      })
    );

    // 2. Save to DB and patch id using the captured index (not length-1)
    ...
    // Patch the DB id onto the AI message in state
    setChats((prev) =>
      prev.map((chat) => {
        if (chat.id !== currentChatId) return chat;
        const msgs = [...chat.messages];
...
// ──────────────────────────────────────────────
//  Full return() for AI Chat App — Professional
//  Drop this inside your component, keep all
...
// WITH this streaming version:
    ...
    setChats((prev) =>
      prev.map((chat) =>
        chat.id === currentChatId
          ? { ...chat, messages: [...updatedMessages, { text: "", sender: "ai" as const, timestamp: aiTimestamp }] }
          : chat
      )
    );
    ...
    // Save completed AI message to DB
    ...
    // Attach the DB id to the message in state
    setChats((prev) =>
      prev.map((chat) => {
        if (chat.id !== currentChatId) return chat;
        const msgs = [...chat.messages];
